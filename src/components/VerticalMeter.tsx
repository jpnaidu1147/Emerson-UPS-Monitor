...
export function VerticalMeter({ title, value, max, unit, colorClass, className }: VerticalMeterProps) {
  const percentage = Math.min(100, Math.max(0, (value / max) * 100));

  return (
    <div className={cn("bg-slate-900 border border-slate-800 rounded-3xl p-6 flex flex-col items-center justify-between min-h-[300px]", className)}>
      <h3 className="text-slate-400 text-[10px] font-bold uppercase tracking-widest text-center h-8 flex items-center justify-center leading-tight">
        {title}
      </h3>
      
      <div className="flex-grow w-full flex items-end justify-center py-4 relative">
         {/* Meter background container */}
         <div className="absolute inset-y-4 w-16 bg-slate-950 rounded-lg overflow-hidden border border-slate-800 shadow-inner">
           ...
           {/* Meter Fill Element */}
           <div
             className={cn("absolute bottom-0 w-full transition-all duration-700 ease-out z-0", colorClass)}
